...
1. 使用当前工作目录名作为 `tmux` session 名称；例如当前目录为 `agent_esp` 时，执行：`tmux new-session -d -s agent_esp`
2. 在该 `tmux` session 中激活虚拟环境：`source ~/.espressif/tools/activate_idf_v5.5.3.sh`
3. 虚拟环境激活成功后，第一次进入该 session 时，执行 `grep -rn TARGET sdkconfig` 判断设备类型是否为 `esp32c5`，如果不是，则执行 `idf.py set-target esp32c5` 切换到正确的设备类型
4. 等待虚拟环境激活成功后，再执行编译命令；不要用 `&&` 将激活和编译写在同一条命令中
5. 执行编译前，先执行 `clear`，避免旧输出干扰判断
6. 在虚拟环境中执行编译：`idf.py build`
